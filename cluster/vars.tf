...
variable "kube" {
    type = object({
        ...
        pool_min_count = optional(number, 2)
        pool_max_count = optional(number, 10)
        pool_vm_size = optional(string, "Standard_D2ads_v5")
        pool_zones = optional(list(string), ["1","2","3"])
        pool_os_disk_type = optional(string, "Ephemeral")
        pool_os_disk_size_gb = optional(number, 64)
        ...
        additional_node_pools = optional(map(object({
            vm_size = optional(string, "Standard_D2ads_v5")
