resource "azurerm_key_vault_secret" "st" {
    ...
    value = jsonencode({
        ...
        "nfs_connection_string_primary" = azurerm_storage_account.nfs[0].primary_connection_string,
        "nfs_connection_string_secondary" = azurerm_storage_account.nfs[0].secondary_connection_string,
        "nfs_account_name" = azurerm_storage_account.nfs[0].name,
        "nfs_resource_group" = azurerm_resource_group.i.name,
        "nfs_key_primary" = azurerm_storage_account.nfs[0].primary_access_key,
        "nfs_key_secondary" = azurerm_storage_account.nfs[0].secondary_access_key,

        "sftp_connection_string_primary" = azurerm_storage_account.sftp[0].primary_connection_string,
        "sftp_connection_string_secondary" = azurerm_storage_account.sftp[0].secondary_connection_string,
        "sftp_account_name" = azurerm_storage_account.sftp[0].name,
        "sftp_resource_group" = azurerm_resource_group.i.name,
        "sftp_key_primary" = azurerm_storage_account.sftp[0].primary_access_key,
        "sftp_key_secondary" = azurerm_storage_account.sftp[0].secondary_access_key,
    })
    tags = {
