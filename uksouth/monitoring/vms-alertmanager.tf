resource "azurerm_availability_set" "alertmanager" {
    name = "${var.environment}-alertmanager-as"
...
resource "azurerm_network_interface" "alertmanager" {
    count = var.alertmanager_count
    name = format("${var.environment}-alertmanager-%02d-nic", count.index + 1)
    location = azurerm_resource_group.rg.location
    resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_virtual_machine" "alertmanager" {
    count = var.alertmanager_count
    name = format("${var.environment}-alertmanager-%02d", count.index + 1)
    location = azurerm_resource_group.rg.location
    resource_group_name = azurerm_resource_group.rg.name
    availability_set_id = azurerm_availability_set.alertmanager.id
    network_interface_ids = [
        element(azurerm_network_interface.alertmanager.*.id, count.index),
    ]
    vm_size = var.alertmanager_vm_size
    ...
    storage_os_disk {
        name = format("${var.environment}-alertmanager-%02d-disk", count.index + 1)
        disk_size_gb = "32"
        caching = "ReadOnly"
    ...
    os_profile {
        computer_name = format("${var.environment}-alertmanager-%02d", count.index + 1)
      ...
      chef:
        ...
        node_name: "${format("${var.environment}-alertmanager-%02d", count.index + 1)}"
...
module "alertmanager_nsg_rules" {
    source = "../../modules/nsg_rules"
    network_security_group_name = "${var.environment}-subnet-04-nsg"
    resource_group_name = azurerm_resource_group.rg.name
    rules = [
        {
...
module "alertmanager_lb_rules" {
    ...
    lb_port = {
        alertmanager_http = ["80", "TCP", "80"],
        alertmanager_https = ["443", "TCP", "443"]
    }
}

resource "azurerm_network_interface_backend_address_pool_association" "alertmanager-bap-pools-assoc" {
    count = var.alertmanager_count
    network_interface_id = element(azurerm_network_interface.alertmanager.*.id, count.index)
