resource "azurerm_availability_set" "grafana" {
    name = "${var.environment}-grafana-as"
    location = azurerm_resource_group.rg.location
    resource_group_name = azurerm_resource_group.rg.name
    platform_fault_domain_count = 2
    managed = true

    tags = var.tags
}

resource "azurerm_network_interface" "grafana" {
    count = var.grafana_count
    name = format("${var.environment}-grafana-%02d-nic", count.index + 1)
    location = azurerm_resource_group.rg.location
    resource_group_name = azurerm_resource_group.rg.name
    depends_on = [azurerm_lb.lb]

    ip_configuration {
        name = "primary"
        subnet_id = azurerm_subnet.subnet.2.id
        private_ip_address_allocation = "Dynamic"
    }
}

resource "azurerm_virtual_machine" "grafana" {
    count = var.grafana_count
    name = format("${var.environment}-grafana-%02d", count.index + 1)
    location = azurerm_resource_group.rg.location
    resource_group_name = azurerm_resource_group.rg.name
    availability_set_id = azurerm_availability_set.grafana.id
    network_interface_ids = [
        element(azurerm_network_interface.grafana.*.id, count.index),
    ]
    vm_size = var.grafana_vm_size
    delete_os_disk_on_termination = true
    delete_data_disks_on_termination = false

    storage_image_reference {
        publisher = "Canonical"
        offer = "UbuntuServer"
        sku = "18.04-LTS"
        version = "latest"
    }

    storage_os_disk {
        name = format("${var.environment}-grafana-%02d-disk", count.index + 1)
        disk_size_gb = "32"
        caching = "ReadOnly"
        create_option = "FromImage"
        managed_disk_type = "StandardSSD_LRS"
    }

    os_profile {
        computer_name = format("${var.environment}-grafana-%02d", count.index + 1)
        admin_username = "terraform"
        custom_data = <<-EOF
      #cloud-config
      write_files:
      - encoding: b64
        content: 
        owner: root:root
        path: /etc/chef/encrypted_data_bag_secret
        permissions: '0600'
      chef:
        install_type: "omnibus"
        force_install: true
        server_url: "https://chef.uksouth.bink.sh:4444/organizations/bink"
        node_name: "${format("${var.environment}-grafana-%02d", count.index + 1)}"
        environment: "${azurerm_resource_group.rg.name}"
        validation_name: "bink-validator"
        validation_cert: |
          -----BEGIN RSA PRIVATE KEY-----
          
          -----END RSA PRIVATE KEY-----
        run_list:
         - "role[grafana]"
        omnibus_url: "https://www.chef.io/chef/install.sh"
        omnibus_version: "15.1.36"
      runcmd:
       - [ chef-client, --chef-license, accept ]
      output: {all: '| tee -a /var/log/cloud-init-output.log'}

    EOF
    }

    os_profile_linux_config {
        disable_password_authentication = true
        ssh_keys {
            path = "/home/terraform/.ssh/authorized_keys"
            key_data = "ssh-rsa  id_bink_azure_terraform"
        }
    }

    tags = var.tags
}

module "grafana_nsg_rules" {
    source = "../../modules/nsg_rules"
    network_security_group_name = "${var.environment}-subnet-03-nsg"
    resource_group_name = azurerm_resource_group.rg.name
    rules = [
        {
            name = "BlockEverything"
            priority = "4096"
            access = "Deny"
        },
        {
            name = "AllowHTTP_AzureLoadBalancerHealthCheck"
            source_address_prefix = "AzureLoadBalancer"
            protocol = "TCP"
            destination_address_prefix = var.subnet_address_prefixes[2]
            destination_port_range = "80"
            priority = "4095"
        },
        {
            name = "AllowHTTPS_AzureLoadBalancerHealthCheck"
            source_address_prefix = "AzureLoadBalancer"
            protocol = "TCP"
            destination_address_prefix = var.subnet_address_prefixes[2]
            destination_port_range = "443"
            priority = "4094"
        },
        {
            name = "AllowSSH_Bastion"
            priority = "500"
            protocol = "TCP"
            destination_port_range = "22"
            source_address_prefix = "192.168.4.0/24"
            destination_address_prefix = var.subnet_address_prefixes[2]
        },
        {
            name = "AllowHTTP_AzureFirewall"
            priority = "100"
            destination_port_range = "80"
            protocol = "TCP"
            source_address_prefix = "192.168.0.0/24"
            destination_address_prefix = var.subnet_address_prefixes[2]
        },
        {
            name = "AllowHTTPS_AzureFirewall"
            priority = "110"
            protocol = "TCP"
            destination_port_range = "443"
            source_address_prefix = "192.168.0.0/24"
            destination_address_prefix = var.subnet_address_prefixes[2]
        }
    ]
}

module "grafana_lb_rules" {
    source = "../../modules/lb_rules"
    loadbalancer_id = azurerm_lb.lb.id
    backend_id = azurerm_lb_backend_address_pool.pools.2.id
    resource_group_name = azurerm_resource_group.rg.name
    frontend_ip_configuration_name = "subnet-03"

    lb_port = {
        grafana_http = ["80", "TCP", "80"],
        grafana_https = ["443", "TCP", "443"]
    }
}

resource "azurerm_network_interface_backend_address_pool_association" "grafana-bap-pools-assoc" {
    count = var.grafana_count
    network_interface_id = element(azurerm_network_interface.grafana.*.id, count.index)
    ip_configuration_name = "primary"
    backend_address_pool_id = azurerm_lb_backend_address_pool.pools.2.id
}
