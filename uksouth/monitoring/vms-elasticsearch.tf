resource "azurerm_availability_set" "elasticsearch" {
  name = "${var.environment}-elasticsearch-as"
...
resource "azurerm_network_interface" "elasticsearch" {
  ...
  name = format("${var.environment}-elasticsearch-%02d-nic", count.index + 1)
  location = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_virtual_machine" "elasticsearch" {
  count = var.elasticsearch_count
  name = format("${var.environment}-elasticsearch-%02d", count.index + 1)
  location = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  ...
  vm_size = var.elasticsearch_vm_size
  ...
  os_profile {
    computer_name = format("${var.environment}-elasticsearch-%02d", count.index + 1)
...
module "elasticsearch_nsg_rules" {
  ...
  rules = [
    ...
    },
    {
      ...
      protocol = "TCP"
      source_address_prefix = "192.168.6.16/28"
      destination_address_prefix = var.subnet_address_prefixes[1]
    },
    {
