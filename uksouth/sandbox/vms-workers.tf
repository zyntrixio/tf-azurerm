...
resource "azurerm_network_interface" "worker" {
    count = var.worker_count
    name = format("${var.environment}-worker-${element(random_pet.workers.*.id, count.index)}-nic")
    location = azurerm_resource_group.rg.location
    resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_virtual_machine" "worker" {
    count = var.worker_count
    name = format("${var.environment}-worker-${element(random_pet.workers.*.id, count.index)}")
    location = azurerm_resource_group.rg.location
    resource_group_name = azurerm_resource_group.rg.name
    ...
    storage_os_disk {
        name = format("${var.environment}-worker-${element(random_pet.workers.*.id, count.index)}-disk")
        disk_size_gb = "32"
        caching = "ReadOnly"
    ...
    os_profile {
        computer_name = format("${var.environment}-worker-${element(random_pet.workers.*.id, count.index)}")
