...
resource "azurerm_cdn_frontdoor_firewall_policy" "traefik" {
    name = "traefik"
    resource_group_name = azurerm_resource_group.i.name
    sku_name = azurerm_cdn_frontdoor_profile.i.sku_name
    enabled = true
...
resource "azurerm_cdn_frontdoor_security_policy" "traefik" {
    name = "traefik"
    cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.i.id

    security_policies {
        firewall {
            cdn_frontdoor_firewall_policy_id = azurerm_cdn_frontdoor_firewall_policy.traefik.id
            association {
                patterns_to_match = ["/*"]
                dynamic domain {
                    for_each = toset([
                        azurerm_cdn_frontdoor_custom_domain.i["uksouth_dev_traefik"].id,
                        azurerm_cdn_frontdoor_custom_domain.i["uksouth_staging_traefik"].id,
                    ])
                    content {
                        cdn_frontdoor_domain_id = domain.key
                    }
